...
echo "ServiceBusCopy Validation (Config). (Patience! This will take a while!)"

# start in script dir 
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
pushd $DIR  > /dev/null

LOGFILE=$DIR/servicebuscopy_config.log 
rm -f $LOGFILE
...
# go into project dir
pushd ../functions/config/ServiceBusCopy > /dev/null
