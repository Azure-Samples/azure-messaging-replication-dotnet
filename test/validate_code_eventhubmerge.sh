...
echo "EventHubMerge Validation (Code). (Patience! This will take a while!)"

# start in script dir 
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
pushd $DIR  > /dev/null

LOGFILE=$DIR/eventhubmerge_code.log 
rm -f $LOGFILE
...
if [ ! $USER_RESOURCE_GROUP ]; then
   ...
   USER_RESOURCE_GROUP='ehmergesmp-'$USER_SUFFIX
fi

USER_LEFT_NAMESPACE_NAME='ehmergesmp-left-'$USER_SUFFIX
USER_RIGHT_NAMESPACE_NAME='ehmergesmp-right-'$USER_SUFFIX
USER_FUNCTIONS_APP_NAME='ehmergesmp-'$USER_SUFFIX
USER_STORAGE_ACCOUNT='ehmergesmp'$USER_SUFFIX
...
az functionapp config appsettings set --name $USER_FUNCTIONS_APP_NAME \
                    --resource-group $USER_RESOURCE_GROUP \
                    --settings "telemetry-left-connection=$cxnstring_left" >> $LOGFILE 2>&1
retval=$?
if [ $retval -ne 0 ]; then
    quit $retval
fi
...
az functionapp config appsettings set --name $USER_FUNCTIONS_APP_NAME \
                    --resource-group $USER_RESOURCE_GROUP \
                    --settings "telemetry-left-consumergroup=$USER_FUNCTIONS_APP_NAME.telemetry" >> $LOGFILE 2>&1
retval=$?
if [ $retval -ne 0 ]; then
    quit $retval
fi
...
az functionapp config appsettings set --name $USER_FUNCTIONS_APP_NAME \
                    --resource-group $USER_RESOURCE_GROUP \
                    --settings "telemetry-right-connection=$cxnstring_right" >> $LOGFILE 2>&1
...
az functionapp config appsettings set --name $USER_FUNCTIONS_APP_NAME \
                    --resource-group $USER_RESOURCE_GROUP \
                    --settings "telemetry-right-consumergroup=$USER_FUNCTIONS_APP_NAME.telemetry" >> $LOGFILE 2>&1
retval=$?
if [ $retval -ne 0 ]; then
    quit $retval
fi
